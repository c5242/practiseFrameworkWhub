...
public class WalletHubHomePage extends CommonMethods
{
	...
	public void hoverOverStarAndValidate()
		{
			...
			Assert.assertTrue("Stars are not highlighted",
			waitForElementToLoad(verifyIfStarsHighlighted).isDisplayed());
	...
	// takes reviewcategory parameter from properties file and selects and
	// verify if selected, this can be read from excel file if there is large
	// data
	...
	// writes review with 200 random charcaters and submit review , returns
	// review string
	...
	public WalletHubProfilePage gotoProfile()
		{
			...
			Assert.assertTrue("Profile Page not displayed",
					waitForElementToLoad(verifyMyprofileDisplayed).isDisplayed());
